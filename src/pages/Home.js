...
const CATEGORY_SHADOW = { boxShadow: "0px 10px 20px 2px rgba(0, 0, 0, 0.25)" };
...
export default function Home() {
  ...
  const handleClickWithToast = useCallback(
    (meal) => {
      addMeal(meal);
      showSuccess();
    },
    ...
    () =>
      meals.map((meal, index) => (
        <MealCard
          ...
          onAdd={handleClickWithToast}
          detailTo={`/meal-notes?id=${meal.id}`}
          variant="shadow"
        />
      )),
    [isMealInCart, handleClickWithToast]
  );
