      ...
      <nav className='flex items-center justify-between'>
        ...
        <div className="flex flex-row-reverse items-center justify-between">
          <ul className={`md:flex flex-row-reverse text-center items-center ${text} ${active ? `py-5 rounded-2xl bg-gray-500 flex flex-col w-full absolute top-[100%] left-0` : `hidden`}`}>
            <Link className="mb-2" toPlace="/" label="الرئيسية" />
            <Link className="mb-2" toPlace="#who-us" label="من نحن" />
            <Link className="mb-2" toPlace="#most-popular" label="الأصناف الأكثر طلباً" />
            <Link className="mb-2" toPlace="#food-list" label="قائمة الطعام" />
